...
import type { EmployeeComboboxProps } from "@/features/employees-management/types/components";
...
export function EmployeeCombobox({
  ...
  const options = useMemo(
    () =>
      members
        .filter((member) => !activeEmployeeIds.includes(member.id))
        .map((member) => ({
          value: member.id,
          label: member.member_name,
          icon: <UserRound className="size-3.5 opacity-70" />,
        })),
    [activeEmployeeIds, members],
  );
