...
import type { ProjectTeamAssignment } from "@/features/projects-management/types/types";
import type { ManagedProjectSummary } from "@/features/team-management/types/types";
...
export async function fetchMemberOtherProjectAssignments(
  memberId: string,
): Promise<ProjectTeamAssignment[]> {
  const { data, error } = await supabase
    .from(DB.OTHER_PROJECT_TEAM_MEMBERS.TABLE)
    .select(DB.OTHER_PROJECT_TEAM_MEMBERS.SELECT)
...
export async function fetchManagedOtherProjects(
  memberId: string,
  ...
  }) as ManagedProjectSummary[];
}

export async function assignMemberToOtherProject(
  memberId: string,
  projectId: string,
): Promise<ProjectTeamAssignment> {
  ...
  if (!project) throw new Error("Other project not found.");
  ...
  if (error) {
    if (error.code === "23505") {
      throw new Error(
        "This team member is already actively assigned to this project.",
      );
    }
...
export async function endOtherProjectTeamAssignment(
  assignmentId: string,
): Promise<void> {
