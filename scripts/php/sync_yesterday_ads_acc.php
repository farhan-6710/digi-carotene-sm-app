 ...
 * Supports Meta Ads and Google Ads (growth_ads_accounts.platform).
...
function syncMetaAdAccountYesterday(array $config, array $account, string $yesterdayDate): void
{
    ...
    if ($accountId === '' || $metaAdAccountId === '' || $accessToken === '') {
        throw new RuntimeException('Missing Meta ad account credentials.');
    }
    ...
    logLine(
        'Done ' . $accountName
        . ' [meta_ads]: campaign_rows=' . $syncedCampaignRows
