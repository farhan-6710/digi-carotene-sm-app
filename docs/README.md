...
- Frontend RBAC: `src/shared/utils/rbac.ts` from `team_members.team_role`. Do not sprinkle `role === "admin"`. Wherever SM executives (or shoot incharge) control an action, admin and manager must too (`isAdminOrManagerRole`) — see [auth-and-features.md](./auth-and-features.md).
